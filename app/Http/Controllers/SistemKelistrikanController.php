...
class SistemKelistrikanController extends Controller
{
    private $menu_model;
    ...
    public function index()
    {
        $data = array(
            ...
            'js'    => '<script src="'.asset('assets/js/page/sistem_kelistrikan.js').'"></script>',
            /* 'js' => highchart()
                    .js_moment()
                    .js_counter()
                    .js_amchart()
    ...
    public function datatable(Request $request)
    {
        ...
        DB::enableQueryLog();
        $data = DB::table('tb_kelistrikan AS ta')
        ...
        ->addColumn('action', function($row) {
            $btn = '';
            $btn_edit = '<a class="mt-1 me-2 btn btn-dim btn-outline-secondary btn-sm" onclick="edit_kelistrikan('.$row->id.')"><em class="icon ni ni-edit"></em><span>Edit</span></a>';
            $btn_delete = '<a class=" mt-1 me-2 btn btn-dim btn-outline-danger btn-sm" onclick="delete_kelistrikan('.$row->id.')"><em class="icon ni ni-trash"></em><span>Delete</span></a>';
    ...
    public function process(Request $request)
    {
        ...
        try{
            ...
            if(!empty($data['id_index_kelistrikan'])){
                //update
                
                $update = DB::table('tb_kelistrikan')
                            ->where('id', $data['id_index_kelistrikan'])
                            ->update([
                                'tgl_input'     => $data['tgl_input'], //format 'Y-m-d
                                'ruang'         => $data['ruang'],
                                'pengkabelan'   => $data['pengkabelan'],
                                'colokan'       => $data['colokan'],
                                'stop_kontak'   => $data['stop_kontak'],
                                'sakelar'       => $data['sakelar'],
                ...
                //insert
                $insert = DB::table('tb_kelistrikan')
                            ->insert([
                                'tgl_input'     => $data['tgl_input'], //format 'Y-m-d
                                'ruang'         => $data['ruang'],
                                'pengkabelan'   => $data['pengkabelan'],
