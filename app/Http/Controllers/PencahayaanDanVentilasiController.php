...
class PencahayaanDanVentilasiController extends Controller
{
    private $menu_model;
    ...
    {
        $data = array(
            'title' => 'Pencahayaan dan Ventilasi',
            'js'    => '<script src="'.asset('assets/js/page/pencahayaan_dan_ventilasi.js').'"></script>',
            /* 'js' => highchart()
                    .js_moment()
                    .js_counter()
                    .js_amchart()
        ...
        view('template.header', $data).
        view("pencahayaan_dan_ventilasi", $data).
    ...
    public function datatable(Request $request)
    {
        ...
        DB::enableQueryLog();
        $data = DB::table('tb_pencahayaan_ventilasi AS ta')
        ...
        ->addColumn('action', function($row) {
            $btn = '';
            $btn_edit = '<a class="mt-1 me-2 btn btn-dim btn-outline-secondary btn-sm" onclick="edit_pencahayaan_ventilasi('.$row->id.')"><em class="icon ni ni-edit"></em><span>Edit</span></a>';
            $btn_delete = '<a class=" mt-1 me-2 btn btn-dim btn-outline-danger btn-sm" onclick="delete_pencahayaan_ventilasi('.$row->id.')"><em class="icon ni ni-trash"></em><span>Delete</span></a>';
    ...
    public function process(Request $request)
    {
        ...
        try{
            ...
            if(!empty($data['id_index_pencahayaan_ventilasi'])){
                //update
                
                $update = DB::table('tb_pencahayaan_ventilasi')
                            ->where('id', $data['id_index_pencahayaan_ventilasi'])
                            ->update([
                ...
                //insert
                $insert = DB::table('tb_pencahayaan_ventilasi')
                            ->insert([
                                'tgl_input'     => $data['tgl_input'], //format 'Y-m-d
                                'ruang'         => $data['ruang'],
                                'lampu'         => $data['lampu'],
    ...
    public function get_data(Request $request)
    {
        ...
        $data = DB::table('tb_pencahayaan_ventilasi')
                ->where('id', $id)
                ->where('status', 1)
                ->first();
