...
class PagarSelasarController extends Controller
{
    private $menu_model;
    ...
    {
        $data = array(
            'title' => 'Pagar Selasar Tangga',
            'js'    => '<script src="'.asset('assets/js/page/pagar_selasar.js').'"></script>',
            /* 'js' => highchart()
                    .js_moment()
                    .js_counter()
                    .js_amchart()
    ...
    public function datatable(Request $request)
    {
        ...
        DB::enableQueryLog();
        $data = DB::table('tb_pagar_selasar AS ta')
        ...
        ->addColumn('action', function($row) {
            $btn = '';
            $btn_edit = '<a class="mt-1 me-2 btn btn-dim btn-outline-secondary btn-sm" onclick="edit_pagar_selasar('.$row->id.')"><em class="icon ni ni-edit"></em><span>Edit</span></a>';
            $btn_delete = '<a class=" mt-1 me-2 btn btn-dim btn-outline-danger btn-sm" onclick="delete_pagar_selasar('.$row->id.')"><em class="icon ni ni-trash"></em><span>Delete</span></a>';
    ...
    public function process(Request $request)
    {
        ...
        try{
            ...
            if(!empty($data['id_index_pagar_selasar'])){
                //update
                
                $update = DB::table('tb_pagar_selasar')
                            ->where('id', $data['id_index_pagar_selasar'])
                            ->update([
                ...
                //insert
                $insert = DB::table('tb_pagar_selasar')
                            ->insert([
                                'tgl_input'     => $data['tgl_input'], //format 'Y-m-d
                                'ruang'         => $data['ruang'],
                                'ram'           => $data['ram'],
    ...
    public function get_data(Request $request)
    {
        ...
        $data = DB::table('tb_pagar_selasar')
                ->where('id', $id)
                ->where('status', 1)
                ->first();
