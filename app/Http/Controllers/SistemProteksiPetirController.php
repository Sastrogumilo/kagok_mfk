...
class SistemProteksiPetirController extends Controller
{
    ...
    public function index()
    {
        $data = array(
            ...
            'js'    => '<script src="'.asset('assets/js/page/sistem_proteksi_petir.js').'"></script>',
            /* 'js' => highchart()
                    .js_moment()
                    .js_counter()
                    .js_amchart()
        ...
        view('template.header', $data).
        view("sistem_proteksi_petir", $data).
    ...
    public function datatable(Request $request)
    {
        ...
        DB::enableQueryLog();
        $data = DB::table('tb_proteksi_petir AS ta')
                ...
                ->select('id',
                        'tgl_input', 
                        'ruang', 
                        'batang_penangkal_petir',
                        'kabel_konduktor_murni',
                        'tempat_pembumian',
                        'penangkal_petir',
                        'keterangan',
                        'tindak_lanjut',
        ...
        ->addColumn('action', function($row) {
            $btn = '';
            $btn_edit = '<a class="mt-1 me-2 btn btn-dim btn-outline-secondary btn-sm" onclick="edit_proteksi_petir('.$row->id.')"><em class="icon ni ni-edit"></em><span>Edit</span></a>';
            $btn_delete = '<a class=" mt-1 me-2 btn btn-dim btn-outline-danger btn-sm" onclick="delete_proteksi_petir('.$row->id.')"><em class="icon ni ni-trash"></em><span>Delete</span></a>';
    ...
    public function process(Request $request)
    {
        ...
        try{
            ...
            if(!empty($data['id_index_proteksi_petir'])){
                //update
                
                $update = DB::table('tb_proteksi_petir')
                            ->where('id', $data['id_index_proteksi_petir'])
                            ->update([
                                'tgl_input'     => $data['tgl_input'], //format 'Y-m-d
                                'ruang'         => $data['ruang'],
                                'batang_penangkal_petir'    => $data['batang_penangkal_petir'],
                                'kabel_konduktor_murni'     => $data['kabel_konduktor_murni'],
                                'tempat_pembumian'          => $data['tempat_pembumian'],
                                'penangkal_petir'           => $data['penangkal_petir'],
                ...
                //insert
                $insert = DB::table('tb_proteksi_petir')
                            ->insert([
                                'tgl_input'     => $data['tgl_input'], //format 'Y-m-d
                                'ruang'         => $data['ruang'],
                                'batang_penangkal_petir'    => $data['batang_penangkal_petir'],
    ...
    public function get_data(Request $request)
    {
        ...
        $data = DB::table('tb_proteksi_petir')
                ->where('id', $id)
                ->where('status', 1)
                ->first();
