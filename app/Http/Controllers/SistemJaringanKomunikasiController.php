...
class SistemJaringanKomunikasiController extends Controller
{
    private $menu_model;
    ...
    {
        $data = array(
            'title' => 'Sistem Jaringan dan Komunikasi',
            'js'    => '<script src="'.asset('assets/js/page/sistem_jaringan_komunikasi.js').'"></script>',
            /* 'js' => highchart()
                    .js_moment()
                    .js_counter()
                    .js_amchart()
        ...
        view('template.header', $data).
        view("sistem_jaringan_komunikasi", $data).
    ...
    public function datatable(Request $request)
    {
        ...
        DB::enableQueryLog();
        $data = DB::table('tb_jaringan AS ta')
        ...
        ->addColumn('action', function($row) {
            $btn = '';
            $btn_edit = '<a class="mt-1 me-2 btn btn-dim btn-outline-secondary btn-sm" onclick="edit_jaringan('.$row->id.')"><em class="icon ni ni-edit"></em><span>Edit</span></a>';
            $btn_delete = '<a class=" mt-1 me-2 btn btn-dim btn-outline-danger btn-sm" onclick="delete_jaringan('.$row->id.')"><em class="icon ni ni-trash"></em><span>Delete</span></a>';
    ...
    public function process(Request $request)
    {
        ...
        try{
            ...
            if(!empty($data['id_index_jaringan'])){
                //update
                
                $update = DB::table('tb_jaringan')
                            ->where('id', $data['id_index_jaringan'])
                            ->update([
                                'tgl_input'     => $data['tgl_input'], //format 'Y-m-d
                                'ruang'         => $data['ruang'],
                                'switch_hub'    => $data['switch_hub'],
                                'kabel_lan'     => $data['kabel_lan'],
                                'indikator_lampu'   => $data['indikator_lampu'],
                                'tes_ping_kecepatan'=> $data['tes_ping_kecepatan'],
