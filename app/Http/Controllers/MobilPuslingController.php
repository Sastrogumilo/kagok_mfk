...
class MobilPuslingController extends Controller
{
    private $menu_model;
    ...
    public function index()
    {
        $data = array(
            'title' => 'Mobil Pusling',
            'js'    => '<script src="'.asset('assets/js/page/mobil_pusling.js').'"></script>',
            /* 'js' => highchart()
                    .js_moment()
                    .js_counter()
                    .js_amchart()
    ...
    public function datatable(Request $request)
    {
        ...
        DB::enableQueryLog();
        $data = DB::table('tb_mobil_pusling AS ta')
        ...
        ->addColumn('action', function($row) {
            $btn = '';
            $btn_edit = '<a class="mt-1 me-2 btn btn-dim btn-outline-secondary btn-sm" onclick="edit_mobil_pusling('.$row->id.')"><em class="icon ni ni-edit"></em><span>Edit</span></a>';
            $btn_delete = '<a class=" mt-1 me-2 btn btn-dim btn-outline-danger btn-sm" onclick="delete_mobil_pusling('.$row->id.')"><em class="icon ni ni-trash"></em><span>Delete</span></a>';
    ...
    public function process(Request $request)
    {
        ...
        try{
            ...
            if(!empty($data['id_index_mobil_pusling'])){
                //update
                
                $update = DB::table('tb_mobil_pusling')
                            ->where('id', $data['id_index_mobil_pusling'])
                            ->update([
                                ...
                                'air_wiper'     => $data['air_wiper'],
                                'karet_wiper'   => $data['karet_wiper'],
                                'kunci_kunci'   => $data['kunci_kunci'],
                                'alat_pengganti_ban'=> $data['alat_pengganti_ban'],
                                'p3k'           => $data['p3k'],
                ...
                //insert
                $insert = DB::table('tb_mobil_pusling')
                            ->insert([
                                'tgl_input'     => $data['tgl_input'], //format 'Y-m-d
                                'kode_kendaraan' => $data['kode_kendaraan'],
                                'karburator'    => $data['karburator'],
