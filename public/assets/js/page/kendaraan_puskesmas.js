...
$('#kode_kendaraan').select2({
    ...
    dropdownParent: $('#modalFormKendaraanPuskesmas'),
         ajax: {
            type: 'GET',
...
let render_table_kendaraan_puskesmas = (data_datatable = []) => {
    ...
    if(data_datatable.length == 0){
        config_datatable = {
            ...
            ajax:  {
                method  : "POST",
                url     : "/kendaraan_puskesmas/datatable",
...
render_table_kendaraan_puskesmas()
...
function clear_form() {
    // Reset input values
    $('#id_index_kendaraan_puskesmas').val('');
...
function add_data(){
    $('#modalFormKendaraanPuskesmas').modal('show');
    $('#tipe_data').html("Tambah Form Data")

}

$('#modalFormKendaraanPuskesmas').on('hidden.bs.modal', function () {
    clear_form()
})
...
$('#form-data-apar').submit(function(e) {
    ...
    if(validate){
        $.ajax({
            url : "/kendaraan_puskesmas/process",  
            data : formData,
            type : "POST",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            cache:false,
            ...
            success: function(response) {
                if(response.status){
                    ...
                    $('#modalFormKendaraanPuskesmas').modal('hide');
...
function delete_kendaraan_puskesmas(id){
    console.log(id)

    $.ajax({
        url : "/kendaraan_puskesmas/delete",  
        data : {id : id},
        type : "POST",
        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        beforeSend: function() {
...
async function edit_kendaraan_puskesmas(id){

    //get data by id
    let data_kendaraan_puskesmas = await axios.post('/kendaraan_puskesmas/get', {id : id}, {
        headers: {
    ...
    let data = data_kendaraan_puskesmas.data.data
    // console.log(data)
    ...
    $('#id_index_kendaraan_puskesmas').val(data.id);
